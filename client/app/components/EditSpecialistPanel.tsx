...
import { ISpecialist, ApprovalStatus } from "../types";
...
function EditSpecialistPanel({ 
  ...
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: Partial<ISpecialist>) => Promise<void>;
  ...
  useEffect(() => {
    if (specialist) {
      setFormData({
        ...
        platform_fee: specialist.platform_fee || 0,
        final_price: specialist.final_price || 0,
        average_rating: specialist.average_rating || 0,
        total_number_of_ratings: specialist.total_number_of_ratings || 0,
        duration_days: specialist.duration_days || 0,
        verification_status: specialist.verification_status || 'pending',
        is_verified: specialist.is_verified || false,
        is_draft: specialist.is_draft || true,
      });
    }
  }, [specialist]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(specialist.id, formData as unknown as Partial<ISpecialist>);
      onClose();
    } catch (error) {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, average_rating: parseFloat(e.target.value) || 0 }))}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900"
                placeholder="4.7"
                disabled={isSubmitting}
              />
            ...
            <div>
              ...
              <input
                type="number"
                min="0"
                value={formData.total_number_of_ratings}
                onChange={(e) => setFormData(prev => ({ ...prev, total_number_of_ratings: parseInt(e.target.value) || 0 }))}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900"
                placeholder="42"
                disabled={isSubmitting}
              />
            ...
            <div className="flex items-center">
              <input
                ...
                disabled={isSubmitting}
              />
              <label htmlFor="edit_is_verified" className="ml-2 text-sm text-gray-700">
                Is Verified
              </label>
            </div>
            ...
            <div className="flex items-center">
              <input
                ...
                disabled={isSubmitting}
              />
              <label htmlFor="edit_is_draft" className="ml-2 text-sm text-gray-700">
                Save as Draft
              </label>
            </div>
          </div>
